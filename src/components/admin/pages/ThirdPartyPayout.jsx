...
const ThirdPartyPayout = () => {
  ...
  const fetchThirdPartyPayouts = async () => {
    ...
    try {
      setLoading(true);
      const [ordersRes, thirdRes, thirdPartyRes, paidRes] = await Promise.all([
        getAllOrders().catch(() => ({ data: [] })),
        getAllThirdParties().catch(() => ({ data: [] })),
        getThirdPartyById(thirdPartyId).catch(() => null),
        getPaidRecords('third_party', { entity_id: thirdPartyId }).catch(() => ({ data: [] }))
      ]);
      ...
      try {
        const stored = localStorage.getItem('payout-thirdparty-paid');
        if (stored) {
  ...
  const handlePay = async (payout) => {
    ...
    try {
      ...
      try {
        ...
        localStorage.setItem('payout-thirdparty-paid', JSON.stringify(list));
      } catch {
        // ignore
      }
    } catch (err) {
      console.error('Error marking third party payout as paid:', err);
    } finally {
  ...
  const thirdPartyExportLabel = thirdParty?.third_party_name || payouts[0]?.thirdPartyName || '';

  const handleExportExcel = () => {
    exportPayoutExcel(
      filteredPayouts.map((p) => ({
        'Order ID': p.orderId || p.id?.split('_')[0] || '—',
        'Third Party Name': p.thirdPartyName,
        'Third Party ID': p.thirdPartyCode,
        'Order Date': payoutExportDate(p.orderDate),
        Amount: p.amount,
        Status: p.paymentStatus || p.status,
      })),
      { sheetName: 'Third Party Payouts', filePrefix: 'ThirdParty_Payout_Detail' }
    );
  };
  ...
  const handleExportPDF = () => {
    exportPayoutPdf({
      ...
      subtitle: buildPayoutExportSubtitle({
        fromDate,
        toDate,
        extra: thirdPartyExportLabel ? `Third Party: ${thirdPartyExportLabel}` : '',
      }),
      headers: ['Order ID', 'Third Party Name', 'Order Date', 'Amount', 'Status'],
      ...
      filePrefix: 'ThirdParty_Payout_Detail',
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#E8F5F1] p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <PayoutFilterBar
          idPrefix="thirdparty-detail-payout"
            ...
            <table className={`${payoutTableBase} min-w-[900px]`}>
              ...
              <tbody className={payoutTbody}>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className={payoutEmptyCell}>No payout records found for this third party</td></tr>
                ) : (
                  paginatedPayouts.map((payout, index) => (
                    <tr key={payout.id} className={payoutRow(index)}>
                      ...
                      <td className={payoutTd}>
                        <div className="font-medium">{payout.thirdPartyName}</div>
        ...
        <PayAllConfirmModal
          ...
          getEntitySecondary={(p) => p.thirdPartyCode}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
            <div className="w-full max-w-md bg-white rounded-xl shadow-xl p-5">
              ...
              <p className="text-sm text-[#6B8782] mb-4">
                {partialModal.payout.thirdPartyName} — Total {formatAmount(partialModal.payout.amount)}
              </p>
