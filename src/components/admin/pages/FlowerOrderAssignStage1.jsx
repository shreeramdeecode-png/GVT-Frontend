...
import { getAssignmentOptions } from '../../../api/orderAssignmentApi';
import { getFlowerOrderAssignment, updateStage1Assignment } from '../../../api/flowerOrderAssignmentApi';
...
const FlowerOrderAssignStage1 = () => {
  ...
  const handleSaveStage1 = async () => {
    try {
      ...
      navigate(`/order-assign/flower/stage2/${id}`, { state: { orderData: orderDetails } });
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-3">
        ...
        <button
          onClick={() => navigate(`/order-assign/flower/stage2/${id}`, { state: { orderData: orderDetails } })}
          className="px-6 py-3 bg-gray-100 text-gray-600 rounded-lg font-medium hover:bg-gray-200 transition-colors"
        >
          Stage 2: Packaging
        </button>
        <button
          onClick={() => navigate(`/order-assign/flower/stage3/${id}`, { state: { orderData: orderDetails } })}
          className="px-6 py-3 bg-gray-100 text-gray-600 rounded-lg font-medium hover:bg-gray-200 transition-colors"
        >
          Stage 3: Delivery
        </button>
        <button
          onClick={() => navigate(`/order-assign/flower/stage4/${id}`, { state: { orderData: orderDetails } })}
          className="px-6 py-3 bg-gray-100 text-gray-600 rounded-lg font-medium hover:bg-gray-200 transition-colors"
        >
          Stage 4: Review
        </button>
                ...
                <tr key={route.routeId || index} className={`hover:bg-gray-50 transition-colors ${route.isRemaining ? 'bg-yellow-50' : ''}`}>
                  ...
                  <td className="px-4 py-4">
                    <select
                      ...
                      value={(Array.isArray(route.labour) ? route.labour[0] : route.labour) || ''}
                      onChange={(e) => {
                        const updatedRoutes = [...deliveryRoutes];
                        updatedRoutes[index].labour = e.target.value ? [e.target.value] : [];
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  >
                    <option value="">Select labour...</option>
                    {assignmentOptions.labours && sortDropdownObjects(assignmentOptions.labours, (l) => l.full_name).map(labour => (
                      <option key={`labour-mobile-${labour.lid}`} value={labour.full_name}>
                        {labour.full_name}
                      </option>
                    ))}
                  </select>
...
export default FlowerOrderAssignStage1;